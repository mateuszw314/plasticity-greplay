...
from feature_dataset import ImageNetDataset
...
def main():
    ...
    for experiment in range(config['num_experiments']):
        ...
        # Training
        train_incremental_tasks(root_path, device, config)
...
def generate_samples(generator: VAE, classifier: ConvClassifier, class_to_generate: int, examples_to_generate: int,
    ...
    """
        ...
        Args:
            ...
            classifier (ConvClassifier): The classifier model.
...
def train_incremental_tasks(dir_path: str, device: torch.device, config: dict) -> None:
    """
        ...
        """
    feature_classifier_params = config['classifier']
    ...
    if 'generator' in config.keys():
        model_vae = VAE(conditional=True, alpha=generator_params['alpha'], continual_backprop=generator_params['cbp'],
                        ...
                        decoder_config=generator_params['decoder_config'], cbp_config=cbp_config, dataset='imagenet').to(device)
        logger.info(f'Generator param size: {u.count_parameters(model_vae)}')
        print('Generator initialized')
    else:
        print('No generator configuration, using exact replay')
    model_classifier = ConvClassifier(input_size=feature_classifier_params['input_size'],
        hidden1=feature_classifier_params['hidden1'], hidden2=feature_classifier_params['hidden2'],
                                      hidden3=feature_classifier_params['hidden3'],
    ...
    for task in range(config['num_tasks']):
        ...
        if task > 1:
            ...
            if 'generator' in config.keys():
                for class_label in previous_classes:
                    samples = generate_samples(model_vae, model_classifier, class_label, config['num_of_exemplars'], device,
        ...
        for eval_task in range(task+1):
            ...
            print(f"Task {task} - Accuracy on task {eval_task}: {accuracy:.2f}%")

            # Store accuracy results
            with open(accuracy_file_path, 'a') as f:
                f.write(f"Task {task} - Accuracy on task {eval_task}: {accuracy:.2f}%\n")
