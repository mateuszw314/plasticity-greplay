...
with mirrored_strategy.scope():
    generator = CIFARVAE(conditional=True, alpha=10.)
...
replay_buffersize = 500

for task in range(10):
    (X_train, Y_train), (X_test, Y_test) = tf.keras.datasets.cifar100.load_data()
    condition = (tf.squeeze(Y_train) < (task+1)*10) & (tf.squeeze(Y_train) >= task*10)
    ...
    #if task > 0:
    #    for digit in range(task*10):
    #        latent_samples = tf.random.normal(mean=generator.prior_means[digit],
    #                                                         stddev=tf.sqrt(tf.exp(generator.prior_logvars[digit])),
    #                                                         shape=(replay_buffersize, 100))
    #        samples = generator.decoder(latent_samples)
    #        X_train = tf.concat((X_train, samples), axis=0)
    #        Y_train = tf.concat((Y_train, tf.constant(digit, shape=(500, 1))), axis=0)
            
    
    print(X_train.shape)
    generator.fit(X_train, Y_train,  epochs=100, batch_size=512, verbose=1)
    



generator.save_weights('generator_class_incremental_without_replay')
